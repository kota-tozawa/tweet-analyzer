...
とりあえず動くが、ちょっとアーキテクチャーがイマイチなので最初から作り直す。\
フロントエンドサーバーとバックエンドサーバー（API）のコードを分離して、別々のレポジトリで管理できるようにして、今の密結合な感じの作りをやめる。\
フロントエンドサーバーを立てるのには、Next.js か Express.js を使い、API を書くに当たっては R の Plumber を使えば良さそう（Shiny は使わないでやる）。\
また、フロントエンドを作るとき、コンポーネントをプレゼンテーションとコンテナに分ける。\
あと、TypeScript を使うようにする。\
それと、取得したツイートデータを DB で管理するようにしてもいいかもしれない。
